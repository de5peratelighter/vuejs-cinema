...
<script>
    export default {
        ...
        data () {
            return {
                days: [0,1,2,3,4,5,6].map(el => this.$moment().add(el, 'days'))
            }
        },
        methods: {
            formatDay (raw) {
                if (raw.isSame(this.$moment(), 'day')) {
                    return 'Today'
                } else {return raw.format('ddd DD/MM')}
            },
            isActive (day) {
            ...
            changeDay (change) {
                let new_day = this.$moment(this.selected).add(change, 'days')
                if (this.days.find(el => new_day.isSame(el,'days'))) {
                    this.selectDay(new_day)
                }
            }
        }
    }
